...
func (p *gameType) Run(input string) []string {
	...
	if strings.HasPrefix(input, "/") {
		...
		if _, exist := p.players[users.LineUser.UserProfile.UserID]; exist {
			if p.players[users.LineUser.UserProfile.UserID].bets > 0 {
				p.dealGate()
				p.showPot()
			}
		}
		...
		if currentPlayer.bets <= 0 {
			...
			bets = helper.Min(helper.Max(1, bets), helper.Min(users.UsersList.Data[users.LineUser.UserProfile.UserID].Money, p.pot2))
